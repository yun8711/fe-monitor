 ...
 * @param opitons
 */
export function on(target: Window | Document, eventName: string, handler: AnyFun, opitons = false): void {
 ...
 * @param keys 需要匹配的属性名
 */
export function groupArray<T, K extends keyof T>(arr: T[], ...keys: K[]): T[][] {
...
export function uuid() {
  ...
  const hexDate = parseInt(`${date.getFullYear()}${pad(date.getMonth() + 1, 2)}${pad(date.getDate(), 2)}`, 10).toString(
    16,
  );
 ...
 * @returns
 */
export function executeFunctions(funList: AnyFun[], through: boolean, args: any): any {
 ...
 * 取消异步执行
 */
export const cancelNextTime = window.cancelIdleCallback || window.cancelAnimationFrame || clearTimeout;
 ...
 */
export function isObjectOverSizeLimit(object: object, limitInKB: number): boolean {
