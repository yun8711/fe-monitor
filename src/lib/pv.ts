...
import { getLocationHref, replaceAop } from '../utils';
import { WEBPAGELOAD } from '../common/constant';
import { _global } from '../utils/global';
import type { BehaviorInfoType } from '../types/global';
import type { AnyObj, VoidFun } from '../types';

const oldURL = getLocationHref();
...
export function initPv() {
  ...
  window.addEventListener('hashchange', () => {
    ...
    // console.log('repetitionRoute', repetitionRoute);
    if (repetitionRoute) return;
    ...
    if (_global.location.hash !== '') {
      const oldHost =
        oldURL.indexOf('#') > 0 // 多页面情况下 history模式刷新还是在pv页面
          ...
          : oldURL;
      if (_global.location.href.slice(0, _global.location.href.indexOf('#')) === oldHost) return;
    }
    lastIsPop = true;
    sendPageView();
  });
...
function sendPageView(option: AnyObj = {}) {
  // console.log('sendPageView', option);
